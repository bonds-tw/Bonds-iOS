...
public enum ErrorCatcher {
    ...
    static func resolvePresenter(explicit: UIViewController?) -> UIViewController? {
        ...
        guard let scene = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.activationState == .foregroundActive || $0.activationState == .foregroundInactive }),
              // The key window may be the temporary unlock window. Error
              // results belong to the wallet underneath, so they survive unlock
              // and never cover the authentication controls.
    ...
    public static func deriveShortError(for error: Error) -> String {
        ...
        // These enums expose only fixed, app-owned recovery messages. Preserve
        // that guidance instead of replacing it with an unexpected-error label.
        if let error = error as? OfficialDocumentInboxError {
            return error.localizedDescription
        }
        if let error = error as? OfficialDocumentPackageError {
            return error.localizedDescription
        }
